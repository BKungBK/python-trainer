...
fn get_submissions(
    state: State<'_, AppState>,
    user_id: String,
) -> Result<Vec<db::SubmissionWithProblem>, String> {
    state
        .db
...
#[tauri::command]
fn get_public_test_cases(
    state: State<'_, AppState>,
    problem_id: String,
) -> Result<Vec<db::TestCase>, String> {
    state
        .db
        .get_public_test_cases(&problem_id)
        .map_err(|e| e.to_string())
}
...
async fn submit_solution(
    ...
    let private_tcs = state
        .db
        .get_private_test_cases(&problem_id)
    ...
    // Persist locally first. Offline mode and transient network failures must never
    // lose a submission; the sync worker can retry unsynced rows later.
    state
        .db
    ...
    if state.supabase.push_submission(&sub).await.is_ok() {
        let _ = state.db.mark_submission_synced(&sub.id);
    }
    ...
    *state.daily_cache.lock().unwrap() = None;
    ...
    let _ = update_daily_progress_after_submit(
        &state.db,
        &state.supabase,
        &user_id,
        &problem_id,
...
async fn get_daily_challenge_inner(
    db: &DbManager,
    supabase: &SupabaseClient,
) -> Result<DailyChallengeInfo, String> {
    ...
    let settings_keys = [
        ...
        "req_data_structures",
        "req_graph",
        "req_dynamic_programming",
        "req_matrix_operations",
        "req_numerical_methods",
        "req_simulation",
        "req_optimization",
        "req_data_parsing",
        "req_statistics",
    ...
    let challenge_uuids = if supabase.is_configured() {
        match supabase.fetch_daily_challenge(&date_str).await {
            ...
            Ok(None) => {
                let seed =
                    (chrono::Utc::now().timestamp_nanos_opt().unwrap_or(12345) & 0xFFFFFFFF) as u32;
                let assigned_ids = db.get_all_assigned_problem_ids().unwrap_or_default();
                let ids = generate_daily_problems_with_seed(
                    seed,
                    &problems,
                    &categories_config,
                    &assigned_ids,
            ...
            Err(e) => {
                ...
                match db.get_daily_challenge(&date_str) {
                    Ok(Some(ids)) => ids,
                    _ => {
                        let seed = (chrono::Utc::now().timestamp_nanos_opt().unwrap_or(12345)
                            & 0xFFFFFFFF) as u32;
                        let assigned_ids = db.get_all_assigned_problem_ids().unwrap_or_default();
                        let ids = generate_daily_problems_with_seed(
                            seed,
...
async fn perform_heartbeat_inner(
    ...
    let (daily_completed, progress_json) = {
        ...
        if let Some(ref info) = *cache {
            ...
            let json =
                serde_json::to_string(&info.category_progress).unwrap_or_else(|_| "{}".to_string());
            (completed, json)
        } else {
            (false, "{}".to_string())
        }
...
fn generate_daily_problems_with_seed(
    ...
    for &(cat, count) in categories_config {
        ...
        for problem in unassigned.iter().take(pick_unassigned) {
            selected_ids.push(problem.id.clone());
        }
        ...
        if picked < target_count {
            ...
            for problem in assigned.iter().take(pick_assigned) {
                selected_ids.push(problem.id.clone());
            }
        }
    }
