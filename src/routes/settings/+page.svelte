<script lang="ts">
  ...
  async function testPythonPath() {
    ...
    try {
      ...
      if (results && results.length > 0 && (results[0].verdict === "Passed" || results[0].verdict === "Accepted")) {
        ...
        testStatus = "Error";
        testError = results[0]?.error || results[0]?.verdict || "การรัน Python ล้มเหลว";
      }
    } catch (e) {
      testStatus = "Error";
      testError = String(e);
  ...
  async function handleRerollDaily() {
    if (!confirm("คุณแน่ใจหรือไม่ที่จะล้างชุดโจทย์วันนี้และสุ่มโจทย์ประจำวันใหม่?")) return;
  ...
  <div class="settings-form">
    ...
    <div style="padding: 8px; margin: -8px; border-radius: var(--radius-sm); transition: background-color var(--duration-state) var(--ease-out-quart);" class:flash-highlight={saveStatus === "Success"}>
      ...
      <div style="display: flex; flex-direction: column; gap: 8px;">
        <p style="font-size: 13px; color: var(--text-secondary); line-height: 1.6;">
          กำหนดเส้นทาง (Path) ไปยังไฟล์โปรแกรม Python โดยค่าเริ่มต้นระบบจะพยายามรันคำสั่ง `python` หรือ `python3` จากตัวแปรสภาพแวดล้อม (Environment Variables) ของระบบโดยอัตโนมัติ
        </p>
        ...
        {#if testStatus === "Testing"}
          <span style="font-size: 11px; color: var(--text-muted);">กำลังตรวจสอบสภาพแวดล้อม Python...</span>
        {:else if testStatus === "Success"}
          <span style="font-size: 11px; color: var(--accent-success); font-weight: 500;">✓ ตรวจสอบการรัน Python สำเร็จแล้ว</span>
        {:else if testStatus === "Error"}
          <span style="font-size: 11px; color: var(--accent-error); font-weight: 500;">✗ ตรวจสอบล้มเหลว: {testError}</span>
        {/if}
      </div>
    </div>
    ...
    <div style="padding: 8px; margin: -8px; border-radius: var(--radius-sm); transition: background-color var(--duration-state) var(--ease-out-quart);" class:flash-highlight={saveStatus === "Success"}>
      ...
      <div style="display: flex; flex-direction: column; gap: 12px;">
        <p style="font-size: 13px; color: var(--text-secondary); line-height: 1.6;">
          ดึงข้อมูลโจทย์ หมวดหมู่ และการตั้งค่าเคสทดสอบล่าสุดลงมาจากฐานข้อมูล Supabase
        </p>
        <div style="display: flex; align-items: center; gap: 12px; margin-top: 4px;">
          ...
          {#if syncStatus === "Syncing"}
            <span style="font-size: 11px; color: var(--text-muted);">กำลังดาวน์โหลด...</span>
          {:else if syncStatus === "Success"}
            <span style="font-size: 11px; color: var(--accent-success); font-weight: 500;">✓ ซิงค์ข้อมูลสำเร็จแล้ว</span>
          {:else if syncStatus === "Error"}
            <span style="font-size: 11px; color: var(--accent-error); font-weight: 500;">✗ ซิงค์ข้อมูลล้มเหลว: {syncError}</span>
          {/if}
        </div>
      </div>
    </div>
    ...
    <div style="padding: 8px; margin: -8px; border-radius: var(--radius-sm); transition: background-color var(--duration-state) var(--ease-out-quart);" class:flash-highlight={saveStatus === "Success"}>
      ...
      <div style="display: flex; flex-direction: column; gap: 12px;">
        <p style="font-size: 13px; color: var(--text-secondary); line-height: 1.6;">
          ล้างชุดโจทย์ประจำวันของวันนี้ออกจากเครื่องและจาก Supabase เพื่อให้ระบบทำการสุ่มเลือกโจทย์ใหม่ที่มีในคลังมาให้ฝึกฝนแทน
        </p>
        <div style="display: flex; align-items: center; gap: 12px; margin-top: 4px;">
          <button 
            class="btn-run" 
          ...
          >
            {rerollStatus === "Rerolling" ? "กำลังรีเซ็ต..." : "รีเซ็ตและสุ่มโจทย์วันนี้ใหม่"}
          </button>
          
          {#if rerollStatus === "Rerolling"}
            <span style="font-size: 11px; color: var(--text-muted);">กำลังส่งคำขอล้างข้อมูล...</span>
          {:else if rerollStatus === "Success"}
            <span style="font-size: 11px; color: var(--accent-success); font-weight: 500;">✓ รีเซ็ตและสุ่มโจทย์ใหม่สำเร็จแล้ว! (กรุณากลับไปที่หน้าความท้าทายประจำวัน)</span>
          {:else if rerollStatus === "Error"}
            <span style="font-size: 11px; color: var(--accent-error); font-weight: 500;">✗ รีเซ็ตล้มเหลว: {rerollError}</span>
          {/if}
        </div>
      </div>
    </div>
    ...
    <div style="padding: 8px; margin: -8px; border-radius: var(--radius-sm); transition: background-color var(--duration-state) var(--ease-out-quart);" class:flash-highlight={saveStatus === "Success"}>
      ...
      <div style="display: flex; flex-direction: column; gap: 12px;">
        ...
        <div style="display: flex; align-items: center; gap: 12px; margin-top: 4px;">
          ...
          >
            {manualUpdateStatus === "Checking" ? "กำลังตรวจเช็ก..." : "ตรวจหาเวอร์ชันใหม่"}
          </button>
          
          {#if manualUpdateStatus === "Checking"}
            <span style="font-size: 11px; color: var(--text-muted);">กำลังติดต่อ GitHub...</span>
          {:else if manualUpdateStatus === "Found"}
            <span style="font-size: 11px; color: var(--accent-blue, #5b9bd5); font-weight: 500;">พบเวอร์ชันใหม่ {appState.updateManifest?.version}! หน้าต่างแจ้งเตือนได้เปิดขึ้นแล้ว</span>
          {:else if manualUpdateStatus === "Latest"}
            <span style="font-size: 11px; color: var(--accent-success); font-weight: 500;">✓ คุณใช้เวอร์ชันล่าสุดอยู่แล้ว (v{appState.currentVersion})</span>
          {:else if manualUpdateStatus === "Error"}
            <span style="font-size: 11px; color: var(--accent-error); font-weight: 500;">✗ เกิดข้อผิดพลาดในการตรวจสอบอัปเดต</span>
          {/if}
        </div>
      </div>
    </div>
    ...
    <div style="display: flex; align-items: center; gap: 14px; padding-top: 4px;">
      ...
      {#if saveStatus === "Success"}
        <span style="font-size: 12px; color: var(--accent-success); font-weight: 500;">✓ บันทึกตั้งค่าสำเร็จแล้ว</span>
      {:else if saveStatus === "Error"}
        <span style="font-size: 12px; color: var(--accent-error); font-weight: 500;">✗ บันทึกตั้งค่าล้มเหลว: {saveError}</span>
      {:else}
        <span style="font-size: 12px; color: var(--text-muted);">การเปลี่ยนแปลงมีผลทันที</span>
      {/if}
    </div>
  </div>
</div>
